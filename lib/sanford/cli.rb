...
module Sanford
  ...
  class Manager
    ...
    def run!(daemonize = false)
      $0 = @process_name
      server = Sanford::Server.new(@host, @server_options)
      server.listen(@ip, @port)

      Signal.trap("TERM"){ server.stop }
      Signal.trap("INT"){ server.halt(false) }
      Signal.trap("USR2"){ self.hot_restart(server) }
    ...
    def hot_restart(server)
      # TODO
      # server.pause
      # ENV["SANFORD_HOST"]        = @name
      # ENV["SANFORD_TCP_SERVER"]  = server.file_descriptor
      # ENV["SANFORD_CONNECTIONS"] = server.connections_file_descriptors.join(',')
      # Kernel.exec("bundle exec sanford start")
    end
